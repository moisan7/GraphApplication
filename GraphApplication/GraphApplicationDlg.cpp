...
BEGIN_MESSAGE_MAP(CGraphApplicationDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON_KRUSKAL_LABELS, &CGraphApplicationDlg::OnBnClickedButtonKruskalLabel)
	ON_BN_CLICKED(IDC_BUTTON_KRUSKAL_TREES, &CGraphApplicationDlg::OnBnClickedButtonKruskalTree)
	...
	ON_BN_CLICKED(IDC_BUTTON_DYNAMIC_PROGRAMMING, &CGraphApplicationDlg::OnBnClickedButtonDynamicProgramming1)
	ON_BN_CLICKED(IDC_BUTTON_BRANCH_BOUND4, &CGraphApplicationDlg::OnBnClickedButtonBranchBound4)
	ON_COMMAND(ID_FILES_LOADGRAPHVISITS, &CGraphApplicationDlg::OnFilesLoadGraphVisits)
	ON_COMMAND(ID_FILES_SAVEGRAPHVISITS, &CGraphApplicationDlg::OnFilesSaveGraphVisits)
	ON_BN_CLICKED(IDC_BUTTON_DYNAMIC_PROGRAMMING_2, &CGraphApplicationDlg::OnBnClickedButtonDynamicProgramming2)
END_MESSAGE_MAP()
...
BOOL CGraphApplicationDlg::OnInitDialog()
{
	...
	m_GraphCtrl.m_DisplayDistances = m_ViewDistances;
	UpdateGrafDisplay();
...
// OnFilesLoadGraphVisits ======================================================

void CGraphApplicationDlg::OnFilesLoadGraphVisits()
{
	...
	if (dlg.DoModal() == IDOK) {
		PrintLog("OnBnClickedButtonLoadGraphVisits %s", dlg.GetPathName());
		try {
			m_Visits.Clear();
			m_Track.Clear();
			m_Track2.Clear();
			...
			cout << "Edges   : " << m_Graph.GetNEdges() << endl;
			m_GraphCtrl.m_DisplayDistances = false;
			m_Visits.Load(dlg.GetPathName().Left(dlg.GetPathName().GetLength()-3)+".VIS");
			cout << "Visits   : " << dlg.GetPathName() << endl;
			cout << "Vertices: " << m_Visits.GetNVertices() << endl;
			PrintLog("OK");
		}
...
// OnFilesSaveGraphVisits ======================================================

void CGraphApplicationDlg::OnFilesSaveGraphVisits()
{
	// TODO: Add your command handler code here
	CFileDialog dlg(false, ".GR", NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
		"Fichero de grafo (*.GR)|*.GR|"
	...
	if (dlg.DoModal() == IDOK) {
		PrintLog("OnBnClickedButtonSaveGraphVisits %s", dlg.GetPathName());
		try {
			m_Graph.Save(dlg.GetPathName());
			m_Visits.Save(dlg.GetPathName().Left(dlg.GetPathName().GetLength() - 3) + ".VIS");
			PrintLog("OK");
		}
		catch (exception& ex) {
			PrintLog("ERROR: %s", ex.what());
...
void CGraphApplicationDlg::OnBnClickedButtonLoadTrack()
{
	...
	if (dlg.DoModal() == IDOK) {
		...
		try {
			...
			cout << "Length : " << m_Track.Length() << endl;
		}
		catch (exception& ex) {
			PrintLog("ERROR: %s", ex.what());
			AfxMessageBox(CString(ex.what()), MB_OK | MB_ICONERROR);
...
// OnBnClickedButtonKruskalLabel ====================================================

void CGraphApplicationDlg::OnBnClickedButtonKruskalLabel()
{
	if (m_Graph.GetNVertices() == 0) {
		AfxMessageBox("El algoritmo de kruskal necesita un grafo para ejecutarse", MB_OK | MB_ICONERROR);
		return;
	}
	PrintLog("OnBnClickedButtonKruskalLabels");
	cout << "KRUSKAL LABEL" << endl;
	try {
		m_Graph.SetDistancesToEdgeLength();
		double t0 = Clock();
...
// OnBnClickedButtonKruskalTree ====================================================

void CGraphApplicationDlg::OnBnClickedButtonKruskalTree()
{
	if (m_Graph.GetNVertices() == 0) {
		AfxMessageBox("El algoritmo de kruskal necesita un grafo para ejecutarse", MB_OK | MB_ICONERROR);
		return;
	}
	PrintLog("OnBnClickedButtonKruskalTrees");
...
// OnBnClickedButtonBranchBound4 ===============================================

void CGraphApplicationDlg::OnBnClickedButtonBranchBound4()
{
	if (m_Graph.GetNVertices() == 0) {
		AfxMessageBox("El algoritmo BranchAndBound4 necesita un grafo para ejecutarse", MB_OK | MB_ICONERROR);
		return;
	}
	if (m_Visits.m_Vertices.empty()) {
		AfxMessageBox("El algoritmo BranchAndBound4 necesita unas visitias para ejecutarse", MB_OK | MB_ICONERROR);
		return;
	}
	PrintLog("OnBnClickedButtonBranchBound4");
	try {
		m_Graph.SetDistancesToEdgeLength();
		cout << "SALESMAN BRANCH&BOUND 4" << endl;
		double t0 = Clock();
		m_Track = SalesmanTrackBranchAndBound4(m_Graph, m_Visits);
		double t = Clock() - t0;
		cout << "TRACK: " << m_Track << endl;
		cout << "TRACK LENGTH: " << m_Track.Length() << endl;
		cout << "Run time: " << t << "seg." << endl;
...
// OnBnClickedButtonDynamicProgramming1 =========================================

void CGraphApplicationDlg::OnBnClickedButtonDynamicProgramming1()
{
	if (m_Graph.GetNVertices() == 0) {
		AfxMessageBox("El algoritmo de programación dinámica necesita un grafo para ejecutarse", MB_OK | MB_ICONERROR);
		return;
	}
	if (m_Visits.m_Vertices.empty()) {
		AfxMessageBox("El algoritmo de programación dinámica necesita unas visitias para ejecutarse", MB_OK | MB_ICONERROR);
		return;
	}
	PrintLog("OnBnClickedButtonDynamicProgramming1");
	try {
		m_Graph.SetDistancesToEdgeLength();
		cout << "SALESMAN DYNAMIC PROGRAMMING 1" << endl;
		double t0 = Clock();
		m_Track = SalesmanTrackDynamicProgramming1(m_Graph, m_Visits);
		double t = Clock() - t0;
		cout << "TRACK: " << m_Track << endl;
		cout << "TRACK LENGTH: " << m_Track.Length() << endl;
		cout << "Run time: " << t << "seg." << endl;
...
void CGraphApplicationDlg::OnBnClickedButtonDynamicProgramming2()
{
	if (m_Graph.GetNVertices() == 0) {
		AfxMessageBox("El algoritmo de programación dinámica necesita un grafo para ejecutarse", MB_OK | MB_ICONERROR);
		return;
	...
	PrintLog("OnBnClickedButtonDynamicProgramming2");
	try {
		m_Graph.SetDistancesToEdgeLength();
		cout << "SALESMAN DYNAMIC PROGRAMMING 2" << endl;
		double t0 = Clock();
		m_Track = SalesmanTrackDynamicProgramming2(m_Graph, m_Visits);
		double t = Clock() - t0;
		cout << "TRACK: " << m_Track << endl;
		cout << "TRACK LENGTH: " << m_Track.Length() << endl;
		cout << "Run time: " << t << "seg." << endl;
